...
const WeatherPage: React.FC = () => {
  ...
  if (loading && !weather) {
    return (
      <div className="max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold mb-6">Weather Conditions</h1>
        <div className="card text-center py-12 text-gray-500">Loading weather data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {driftAssessment && (
        <div
          className={`p-6 rounded-lg border-2 ${getRecommendationColor(weather.sprayRecommendation)}`}
        >
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="flex items-center gap-3">
                ...
                <button onClick={loadWeather} className="btn-secondary text-sm py-1.5 px-4">
                  Refresh
                </button>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <MetricBox
              ...
              sub={weather.gustFactor > 0 ? `+${Math.round(weather.gustFactor)} mph` : undefined}
        ...
        {driftAssessment && (
          <div className="card">
            <h3 className="font-semibold text-lg mb-4">Drift Risk Factors</h3>
            <div className="space-y-3">
              {Object.entries(driftAssessment.factors).map(([factor, data]) => (
                <div key={factor} className="flex items-center gap-3">
                  <div
                    className={`w-3 h-3 rounded-full flex-shrink-0 ${getScoreColor(data.score)}`}
                  />
          ...
          <div className="card">
            ...
            {driftAssessment.recommendations.length === 0 ? (
              <div className="text-center py-8 text-green-600">
                <p className="font-medium text-lg">Excellent spray conditions!</p>
                <p className="text-sm mt-1">No special precautions needed</p>
              </div>
            ) : (
